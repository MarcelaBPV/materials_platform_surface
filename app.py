    ...
    if uploaded:
        try:
            ...
            if any(col in df_new.columns for col in ["wavenumber_cm1", "intensity_a"]):
                st.error("⚠️ Este arquivo contém dados experimentais. Use a aba 2️⃣ Ensaios.")
                st.stop()
    ...
    if uploaded_file:
        try:
            ...
            if tipo == "Raman":
                ...
                # ✅ Criação do espectro compatível com a nova versão
